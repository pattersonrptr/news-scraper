...
- **Toast notifications (Phase 10)**: `sonner` v2 is used for all user-facing toast feedback. `<Toaster richColors position="bottom-right" />` is mounted in `frontend/src/app/(main)/layout.tsx`. All source mutations (create, update, delete, toggle active) call `toast.success()` / `toast.error()`.
- **shadcn Dialog**: Installed via `shadcn@latest add dialog`. The component lives at `frontend/src/components/ui/dialog.tsx` and uses `import { Dialog as DialogPrimitive } from "radix-ui"` (bundle import). Used on the Sources page for delete confirmation.
- **Docker anonymous volume for node_modules**: `docker-compose.yml` uses an anonymous volume `/app/node_modules` to shadow the bind-mount `./frontend:/app`, so the container's `node_modules` (built by `npm ci` in the image) is not overwritten by the host. **Consequence**: after adding new npm packages, the anonymous volume must be recreated or it will serve stale `node_modules`. Fix: `docker compose down --volumes && docker compose up -d` (named volumes `postgres_data`, `redis_data`, `celerybeat_data` are preserved). See ADR-012.
